...
Pass `--quiet`, as in `clikernel-mcp --quiet`, to omit startup output from replies. Python startup code still runs. Pass `--cfgdir <dir>` to read `startup.py`, `inspectors.py`, and `gateways.toml` from `<dir>` in place of `~/.config/clikernel/`.
