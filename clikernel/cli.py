...
def _startup_block(shell):
    ...
    def runner(_src):
        ...
        shell.user_ns.pop('__file__', None)  # %run -i leaves it behind; a notebook kernel has no __file__
...
def _inspect(shell, inspectors, code):
    "Run each inspector on the cell; return concatenated notes. An inspector may raise RuleBlock to block the cell."
...
def _make_shell():
    ...
    shell.register_magic_function(_nbrun_magic(shell), 'line', 'nbrun')
