...
- `execute` — the one composite: send `execute_request` with `allow_stdin=False`, collect iopub until idle, convert with `fastcore.nbio.msgs2outs`, render with `render_outs` (concise text, ANSI-stripped, capped tracebacks).
